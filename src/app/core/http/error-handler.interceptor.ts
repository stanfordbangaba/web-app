...
export class ErrorHandlerInterceptor implements HttpInterceptor {
  ...
  private handleError(response: HttpErrorResponse): Observable<HttpEvent<any>> {
    ...
    if (response.error) {
      ...
      if (response.error.responseCode === 'VALIDATION_ERROR' ||
        response.error.responseCode === 'ERROR' ||
        response.error.responseCode === 'FAILED') {
        errorMessage = response.error.narrative;
    ...
    if (status === 401 || (environment.oauth.enabled && status === 400)) {
      ...
      this.alertService.alert({type: 'Unknown Error', message: 'Configuration Server Error. Please try again later.'});
    }
